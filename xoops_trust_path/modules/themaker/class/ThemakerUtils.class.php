...
class Themaker_Utils
{
    ...
    **/
    ...
    {
        $action = isset($action) ? $action : Themaker_AuthType::VIEW;
        ...
        if($role=='cat'){
            ...
            XCube_DelegateUtils::call($delegateName, new XCube_Ref($idList), $accessController->dirname(), self::getActor($dirname, Themaker_AuthType::VIEW), Legacy_Utils::getUid(), $categoryId);
        }
        elseif($role=='group'){
            $delegateName = 'Legacy_Group.'.$accessController->dirname().'.GetGroupIdListByAction';
            XCube_DelegateUtils::call($delegateName, new XCube_Ref($idList), $accessController->dirname(), $dirname, 'page', Themaker_AuthType::VIEW);
        }
    ...
    **/
     ...
     * 
     * @return  Themaker_AbstractAccessController
    **/
    public static function getAccessControllerObject(/*** string ***/ $dirname, /*** string ***/ $dataname)
    {
        ...
        if(! $module){
            require_once THEMAKER_TRUST_PATH . '/class/NoneAccessController.class.php';
            $accessController = new Themaker_NoneAccessController($server, $dirname, $dataname);
        ...
        switch($role){
        case 'cat':
            require_once THEMAKER_TRUST_PATH . '/class/CatAccessController.class.php';
            $accessController = new Themaker_CatAccessController($server, $dirname, $dataname);
            break;
        case 'group':
            require_once THEMAKER_TRUST_PATH . '/class/GroupAccessController.class.php';
            $accessController = new Themaker_GroupAccessController($server, $dirname, $dataname);
            break;
        case 'none':
        default:
            require_once THEMAKER_TRUST_PATH . '/class/NoneAccessController.class.php';
    ...
    **/
    public static function getActor(/*** string ***/ $dirname, /*** string ***/ $action)
    {
        ...
        switch($action){
            case Themaker_AuthType::VIEW:
                return trim($authType[0]);
                break;
            case Themaker_AuthType::POST:
                return trim($authType[1]);
                break;
            case Themaker_AuthType::MANAGE:
